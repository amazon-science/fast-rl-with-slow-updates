...
class DQNAgent(object):
  ...
  def __init__(self,
    ...
    if mu > 0.0:
        if hasattr(optimizer, '_learning_rate'):
            mu = optimizer._learning_rate / mu
    ...
    with tf.device(tf_device):
      ...
      self._sync_qt_ops = self._build_sync_ops()
  ...
  def _build_train_ops(self):
    ...
    target = tf.stop_gradient(self._build_target_q_ops())

    # compute loss
    loss = tf.compat.v1.losses.huber_loss(target, replay_chosen_q, reduction=tf.losses.Reduction.NONE)
    if self.summary_writer is not None:
    ...
    prox_iter_list = []
    if self.mu > 0.0:
      prox_iter_list = self._build_prox_ops()

    # update ops
    with tf.control_dependencies(prox_iter_list):
      loss_step_op = self.optimizer.apply_gradients(grads_and_vars)
    return loss_step_op
  ...
  def _build_prox_ops(self):
    ...
    for (w_online, w_target) in zip(trainables_online, trainables_target):
      prox_iter_ops.append(w_online.assign(w_online * (1 - self.mu) + tf.stop_gradient(w_target) * self.mu, use_locking=True))
    return prox_iter_ops
  ...
  def _train_step(self):
    ...
    if self._replay.memory.add_count > self.min_replay_history:
      if self.training_steps % self.update_period == 0:
        ...
        loss = self._sess.run(self._loss_ops)
        loss = np.mean(loss)
